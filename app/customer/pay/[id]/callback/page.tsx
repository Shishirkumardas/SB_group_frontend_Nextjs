...
export default function PaymentCallbackPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [masterDataId, setMasterDataId] = useState<string | null>(null);
    ...
    const handleExecutePayment = async (paymentID: string) => {
        try {
            const res = await fetch(`http://localhost:8080/api/bkash/callback?masterDataId=${id}&paymentID=${paymentID}&status=success`, {
                method: "GET",
                credentials: "include",
                headers: {
